...
def main():
	...
	try:
		...
		elif "att" in x.lower()[0:3] or "attend" in x.lower()[0:6]:
			try:
				l = x.split()
				l.pop(0)
				final_attend = list(map(int,l))
				c_a = final_attend[0]  #Total Classes attended
				c_h = final_attend[1]  #Total no. of classes taken
				perc = final_attend[2] #Min. percentage of attendance required
				spot_1 = (c_a/c_h)*100
				c_new = (c_h*perc)//100
				spot_2 = (c_a - c_new)
				if spot_2<1:
				...
				if spot_2==0:
					tk.Tk().withdraw()
					showinfo("ATTENDANCE DETAILS!","Your attendace is {:.2f}.\nYou can skip {} classes!!".format(float(spot_1), spot_2))
				else:
					tk.Tk().withdraw()
					showinfo("ATTENDANCE DETAILS!","Your attendace is {:.2f}.\nYou can skip {} classes approximately!!".format(float(spot_1), spot_2))
			except:
				tk.Tk().withdraw()
				showerror("LISTARY PY","Entered numbers are not pure integers/numbers. Please try again!")
		elif new_path!="":
			os.chdir(new_path)
			try:
